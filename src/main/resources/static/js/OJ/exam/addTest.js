...
if (testInfo.id != 'add'){
    selectedQueList = testInfo.selectedQueList;
    selectedClassList = testInfo.selectedClassList;
    selectedJroomList = testInfo.selectedJroomList;
    testInfo.testInfo.start = formatTime(testInfo.testInfo.start);
    testInfo.testInfo.end = formatTime(testInfo.testInfo.end);
}
toastr.options = {
    "closeButton": true,
    "debug": false,
...
function loadLayerDate() {
    ...
    var endTime;
    if (testInfo.id != 'add'){
        startTime = testInfo.testInfo.start;
        endTime = testInfo.testInfo.end;
        $('#testStartTime').val(startTime);
        $('#testEndTime').val(endTime);
    }else{
        startTime = laydate.now();
        endTime = laydate.now();
        $('#testStartTime').val(laydate.now(0, 'YYYY-MM-DD hh:mm:ss'));
        $('#testEndTime').val(laydate.now(7, 'YYYY-MM-DD hh:mm:ss'));
    };
...
function loadExpreInfo() {
    $("#testName").val(testInfo.testInfo.name);
    $("#testName").attr("disabled","disabled");
    $('#testDesc').code(testInfo.testInfo.description);
    if ("" == testInfo.testInfo.is_ip){
        $("[name='is_ip']").removeAttr("checked");
    }
    if ("" == testInfo.testInfo.only_ip){
...
function loadPreSelectQuestion() {
    $.ajax({
        type: "POST",
        url: "/testMn/loadPreSelectQuestion",
        dataType: "json",
...
function saveOrUpdateTest() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            "testInfo":{
                "name":$("#testName").val(),
                "start":$("#testStartTime").val(),
                "end":$("#testEndTime").val(),
                "description":$("#testDesc").code(),
